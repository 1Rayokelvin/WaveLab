...
from .spectra import SpatialSpectra, SpectralSpectra
...
class SerializableConfig:
    ...
    def _serialize_callable(self, func):
        ...
        for cls in [SpatialSpectra, SpectralSpectra]:
...
class SourceConfig(SerializableConfig):
    ...
    Contains the physical and sampling parameters of the source.
    ...
    pol_vect : Tuple[complex, complex]
        Jones vector (E1, E2) defining base polarization. 
        Is defined on beam_axis plane, will be normalized.

    2) Sampling Parameters

    num_modes : int
        Number of plane wave modes used.
    sampling_method: 'fibonacci' | 'gaussian'
        Method used to sample modes on unit sphere.
    sampling_parameter : float (radians)
        1) For fibonacci sampling, it is
           maximum allowed angle between a mode and beam_axis.
        2) For gaussian sampling, it is standard deviation.
    """
    ...
    num_modes: int = 6000
    sampling_method: Literal["fibonacci", "gaussian"] = "fibonacci"
    sampling_param: float = np.pi/2
    ...
    def validate(self):
        ...
        if not self.sampling_method in ["fibonacci", "gaussian"]:
            raise ValueError("sampling must be 'fibonacci' or 'gaussian'.")
        
        self.sampling_param = _check_scalar(self.sampling_param, "source.sampling_param", float)
        if self.sampling_method == "gaussian":
            if self.sampling_param > np.pi/2:
                warnings.warn("Gaussian sampling std > pi/2, "\
                              "fibonacci sampling works better in this regime!.")
            elif self.sampling_param <0:
                raise ValueError("Gaussian sampling std must be positive.")
        if self.sampling_method == "fibonacci" and not 0 <= self.sampling_param <= np.pi:
            raise ValueError("For fibonacci sampling, maximum angle between " \
                             "beam_axis and mode should be in 0, pi.")
...
class SpatialConfig(SerializableConfig):
    ...
    def tophat(self, k_perp_max: float = 1.0):
        ...
        self.profile = SpatialSpectra.tophat
    ...
    def laguerre_gauss(self, *, p: int = 0, l: int = 0, sigma_k_perp: float = 0.5):
        ...
        self.profile = SpatialSpectra.laguerre_gauss
    ...
    def hermite_gauss(self, *, l: int = 0, m: int = 0, sigma_k_perp: float = 0.5):
        ...
        self.profile = SpatialSpectra.hermite_gauss
    ...
    def bessel_gauss(self, *, theta_deg: float = 10.0, sigma_theta: float = 0.05, l: int = 0):
        ...
        self.profile = SpatialSpectra.bessel_gauss
...
class SpectralConfig(SerializableConfig):
    ...
    """
    profile: Callable = field(default=SpectralSpectra.uniform)
    params: Dict[str, Any] = field(default_factory=dict)

    def uniform(self):
        """Set spectral envelope to Uniform (no scaling applied)."""
        self.profile = SpectralSpectra.uniform
        self.params = {}
        return self

    def gaussian(self, center: float = 8.0, sigma: float = 0.5):
        """Set spectral envelope to a Gaussian distribution."""
        self.profile = SpectralSpectra.gaussian
    ...
    def lorentzian(self, center: float = 8.0, gamma: float = 0.5):
        ...
        self.profile = SpectralSpectra.lorentzian
    ...
    def tophat(self, center: float = 8.0, width: float = 1.0):
        ...
        self.profile = SpectralSpectra.tophat
